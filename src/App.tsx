...
function stageLabelsFor(labels: AppLabels): Record<PersonStage, string> {
  return {
    new: 'Novo',
    contact_authorized: 'Contato autorizado',
    care_done: `${labels.care} realizado`,
    group_connected: `Conectado a ${labels.group}`,
    discipleship_active: `${labels.discipleship} ativo`,
    integrated: 'Integrado',
  }
}
const roleLabels: Record<Role, string> = {
  owner: 'Proprietário', pastor: 'Pastor', coordinator: 'Coord. Presença',
  care: 'Cuidado', group_leader: 'Líder de grupo', discipler: 'Discipulador',
  data_admin: 'Admin. de dados',
}
const labelFieldNames: Record<keyof AppLabels, string> = {
  reception: 'Recepção e presença',
  table: 'Mesa e convivência',
  care: 'Cuidado e conexão',
  group: 'Pequenos grupos',
  discipleship: 'Discipulado inicial',
}
const scripts = [
...
function messageSuggestions(person: Person, intent: MessageIntent, labels: AppLabels) {
  const name = firstName(person.name)
  if (intent === 'group') return [
    `Oi, ${name}! Foi muito bom conversar com você. Temos um encontro do ${labels.group} com pessoas da sua região, em um ambiente simples de amizade, Bíblia e oração. Posso te explicar como funciona?`,
    `Olá, ${name}! Lembrei de você porque o ${labels.group} pode ser um bom próximo passo para criar vínculos. O convite é sem compromisso. Quer conhecer?`,
    `Oi, ${name}! Gostaríamos de convidar você para conhecer o ${labels.group}. É um encontro acolhedor, com conversa e oração. Se quiser, envio os detalhes.`,
  ]
  if (intent === 'discipleship') return [
    `Oi, ${name}! Queremos caminhar mais perto de você. O ${labels.discipleship} reúne conversas individuais sobre fé, Bíblia, oração e vida com Jesus. Gostaria de conhecer a proposta?`,
    `Olá, ${name}! Pensamos em convidar você para o ${labels.discipleship}: um caminho de encontros, no seu ritmo, com alguém ao seu lado. Posso te contar melhor?`,
    `Oi, ${name}! O próximo passo que queremos oferecer é o ${labels.discipleship}, um acompanhamento pessoal e simples para fortalecer sua caminhada com Cristo. Você gostaria de conversar sobre isso?`,
  ]
...
function programText(text: string, labels: AppLabels) {
  return text
    .replaceAll('Cuidado e Conexão', labels.care)
    .replaceAll('Mesa Aberta', labels.table)
    .replaceAll('Casa de Paz', labels.group)
    .replaceAll('Casa piloto', `${labels.group} piloto`)
    .replaceAll('Casas', labels.group)
    .replaceAll('Casa', labels.group)
    .replaceAll('Raiz', labels.discipleship)
}
...
export default function App() {
  ...
  return <div className="app-shell" style={{ '--tenant-primary': organization.primaryColor, '--tenant-accent': organization.accentColor } as React.CSSProperties}>
    <aside className={`sidebar ${mobileNav ? 'sidebar-open' : ''}`}>
      <button className="brand" onClick={() => go('home')}><img src="/icon.svg" alt="" /><span><strong>{PRODUCT_NAME}</strong><small>{organization.ministryName} · {organization.name}</small></span></button>
    ...
    <main>
      <header className="topbar">
        ...
        <div className="mobile-brand"><img src="/icon.svg" alt="" /><strong>{PRODUCT_NAME}</strong></div>
        <div className="page-context"><span>{organization.ministryName}</span><i /><strong>{currentTitle}</strong><em>{ecosystem.isCloud ? 'nuvem' : 'demonstração'}</em></div>
      ...
      <div className="content">
        ...
        {view === 'home' && <Dashboard people={scopedPeople} groups={scopedGroups} discipleships={scopedDiscipleships} pending={pendingCare} labels={labels} ministryName={organization.ministryName} go={go} open={setSelectedPerson} />}
        {view === 'implementation' && <Implementation done={weekDone} setDone={setWeekDone} labels={labels} />}
        ...
        {view === 'people' && <People people={scopedPeople} labels={labels} congregationName={congregationName} open={setSelectedPerson} add={() => setShowAddPerson(true)} />}
        {view === 'care' && <Care people={scopedPeople} labels={labels} congregationName={congregationName} contact={(person) => setMessageDraft({ person, intent: 'current' })} />}
        {view === 'groups' && <Groups groups={scopedGroups} meetings={inScope(groupMeetings)} requests={inScope(joinRequests)} labels={labels} addGroup={() => { const unit = congregationId === 'all' ? congregations[0]?.id : congregationId; const name = `Novo ${labels.group}`; setGroups((current) => [...current, { id: crypto.randomUUID(), organizationId: organization.id, congregationId: unit, name, leader: 'Definir líder', host: 'Definir anfitrião', apprentice: 'Em formação', neighborhood: 'Definir bairro', weekday: 'Quinzenal', time: '20h', capacity: 12, participants: 0, health: 'healthy' }]); log(`${labels.group} criado`, name) }} addMeeting={(group) => { const meeting = { id: crypto.randomUUID(), organizationId: organization.id, congregationId: group.congregationId, groupId: group.id, date: new Date().toISOString().slice(0,10), attendance: group.participants, newcomers: 0, followUpAuthorized: 0, pastoralFlag: false, operationalNote: 'Relatório mínimo registrado.' }; setGroupMeetings((current) => [meeting, ...current]); log(`Relatório de ${labels.group} registrado`, group.name) }} accept={(request) => { setJoinRequests((current) => current.map((item) => item.id === request.id ? { ...item, status: 'accepted', groupId: scopedGroups[0]?.id } : item)); log('Pedido de entrada aceito', request.personName) }} />}
        {view === 'discipleship' && <Discipleship labels={labels} items={scopedDiscipleships} team={team} advance={(item) => { const next = Math.min(item.meeting + 1, 7); setDiscipleships((current) => current.map((existing) => existing.id === item.id ? { ...existing, meeting: next, completedMeetings: [...new Set([...(existing.completedMeetings ?? []), existing.meeting])], status: next === 7 ? 'completed' : 'active', nextMeeting: next === 7 ? 'Ciclo concluído' : 'Agendar próximo encontro' } : existing)); log(`Encontro do ${labels.discipleship} concluído`, item.person) }} />}
    ...
    {selectedPerson ? <PersonSheet person={selectedPerson} labels={labels} congregationName={congregationName} close={() => setSelectedPerson(null)} update={(patch, action) => updatePerson(selectedPerson.id, patch, action)} contact={(intent) => setMessageDraft({ person: selectedPerson, intent })} requestRetention={(type) => { setRetention((current) => [{ id: crypto.randomUUID(), personName: selectedPerson.name, type, status: 'open', requestedAt: new Date().toISOString().slice(0,10) }, ...current]); log('Solicitação de privacidade criada', selectedPerson.name, 'restricted') }} /> : null}
    {showSearch ? <GlobalSearch people={scopedPeople} groups={scopedGroups} labels={labels} close={() => setShowSearch(false)} openPerson={(person) => { setShowSearch(false); setSelectedPerson(person) }} go={go} /> : null}
    {messageDraft ? <MessageComposer person={messageDraft.person} intent={messageDraft.intent} labels={labels} close={() => setMessageDraft(null)} sent={() => { updatePerson(messageDraft.person.id, { stage: messageDraft.intent === 'group' ? 'group_connected' : messageDraft.intent === 'discipleship' ? 'discipleship_active' : 'care_done', contactStatus: 'sent', lastContactAt: new Date().toISOString(), nextAction: messageDraft.intent === 'group' ? `Confirmar participação em ${labels.group}` : messageDraft.intent === 'discipleship' ? `Definir responsável e primeiro encontro de ${labels.discipleship}` : `Convidar para ${labels.group}`, dueLabel: 'Em até 7 dias' }, 'Mensagem preparada para contato'); setMessageDraft(null) }} /> : null}
  </div>
}
...
function Dashboard({ people, pending, groups, discipleships, labels, ministryName, go, open }: { people: Person[]; pending: Person[]; groups: SmallGroup[]; discipleships: DiscipleshipType[]; labels: AppLabels; ministryName: string; go: (view: View) => void; open: (person: Person) => void }) {
  const stageLabels = stageLabelsFor(labels)
  ...
  return <>
    ...
    <section className="metric-grid">
      ...
      <Metric icon={Leaf} label={`Relações de ${labels.discipleship}`} value={discipleships.filter((item) => item.status === 'active').length} detail="máximo saudável: 3 por responsável" tone="green" onClick={() => go('discipleship')} />
    </section>
    ...
    <section className="culture-card"><span className="culture-mark"><img src="/icon.svg" alt="" /></span><div><span className="eyebrow">{ministryName.toUpperCase()}</span><blockquote>Pessoas ao lado de pessoas, até que criem raízes em Cristo e aprendam a fazer o mesmo por outras.</blockquote></div></section>
  </>
}
...
function Implementation({ done, setDone, labels }: { done: Record<number, number[]>; setDone: React.Dispatch<React.SetStateAction<Record<number, number[]>>>; labels: AppLabels }) {
  ...
  return <><Intro eyebrow="IMPLANTAÇÃO GUIADA" title="Sete semanas para formar uma cultura" description="Treinamento bíblico, simulação, prática real e responsáveis claros." action={<Pill tone="gold">{completed} de {total} tarefas</Pill>} />
    ...
    <section className="implementation-layout"><div className="panel week-list">{implementationWeeks.map((item) => <button className={item.week === selected ? 'selected' : ''} key={item.week} onClick={() => setSelected(item.week)}><span className={`week-number ${done[item.week]?.length === item.tasks.length ? 'done' : ''}`}>{done[item.week]?.length === item.tasks.length ? <Check /> : item.week}</span><div><small>SEMANA {item.week}</small><strong>{programText(item.title, labels)}</strong><span>{done[item.week]?.length ?? 0}/{item.tasks.length} tarefas concluídas</span></div><ChevronRight /></button>)}</div>
      <div className="panel week-detail"><span className="eyebrow">SEMANA {week.week}</span><h2>{programText(week.title, labels)}</h2><p>Encontro de 40–45 minutos: oração, fundamento bíblico, treinamento, simulação e prática.</p><div className="agenda"><h3>Checklist da semana</h3>{week.tasks.map((task, index) => <button className={done[selected]?.includes(index) ? 'checked' : ''} key={task} onClick={() => toggle(index)}><span>{done[selected]?.includes(index) ? <Check /> : index + 1}</span><p>{programText(task, labels)}</p></button>)}</div><div className="week-callout"><Sparkles /><div><strong>Prática real</strong><p>{programText(week.practice, labels)}</p></div></div><div className="week-rhythm"><span>0–5 Oração</span><span>5–13 Bíblia</span><span>13–28 Treino</span><span>28–38 Simulação</span><span>38–45 Envio</span></div></div>
    </section>
  </>
}
...
function Care({ people, labels, congregationName, contact }: { people: Person[]; labels: AppLabels; congregationName: (id: string) => string; contact: (person: Person) => void }) {
  ...
  return <><Intro title={labels.care} description="Contato em aproximadamente 24 horas, no máximo 48, com consentimento e limites." />
    <section className="care-layout"><div className="care-board">{[
      ...
      <aside className="panel script-panel"><span className="eyebrow">SCRIPTS DE APOIO</span><div className="script-tabs">{scripts.map(([title], index) => <button className={script === index ? 'active' : ''} onClick={() => setScript(index)} key={title}>{title}</button>)}</div><MessageCircle /><h2>{scripts[script][0]}</h2><blockquote>“{programText(scripts[script][1], labels)}”</blockquote><p>Adapte à relação. Nunca transforme cuidado em mensagem em massa.</p><div className="safe-callout"><AlertTriangle /><div><strong>Encaminhe imediatamente</strong><p>Risco de autoagressão, violência, abuso, emergência, crime ou abuso de poder.</p></div></div></aside>
    </section>
...
function Groups({ groups, meetings, requests, labels, addGroup, addMeeting, accept }: { groups: SmallGroup[]; meetings: GroupMeeting[]; requests: JoinRequest[]; labels: AppLabels; addGroup: () => void; addMeeting: (group: SmallGroup) => void; accept: (request: JoinRequest) => void }) {
  ...
  return <><Intro title={labels.group} description="Bíblia, conversa, oração e vínculo — grupos saudáveis de 6–10 pessoas, máximo 12." action={<button className="primary" onClick={addGroup}><Plus /> Novo grupo</button>} />
    <div className="tabs"><button className={tab === 'groups' ? 'active' : ''} onClick={() => setTab('groups')}>{labels.group}</button><button className={tab === 'requests' ? 'active' : ''} onClick={() => setTab('requests')}>Pedidos de entrada <b>{requests.filter((item) => item.status === 'pending').length}</b></button><button className={tab === 'reports' ? 'active' : ''} onClick={() => setTab('reports')}>Relatórios mínimos</button></div>
...
function MessageComposer({ person, intent, labels, close, sent }: { person: Person; intent: MessageIntent; labels: AppLabels; close: () => void; sent: () => void }) {
  const suggestions = messageSuggestions(person, intent, labels)
...
function PersonSheet({ person, labels, congregationName, close, update, contact, requestRetention }: { person: Person; labels: AppLabels; congregationName: (id: string) => string; close: () => void; update: (patch: Partial<Person>, action: string) => void; contact: (intent: MessageIntent) => void; requestRetention: (type: RetentionRequest['type']) => void }) {
  const stageLabels = stageLabelsFor(labels)
  ...
  return <div className="modal-backdrop sheet-backdrop" onMouseDown={close}>
    <aside className="person-sheet" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="sheet-section"><h3>Próximos passos</h3>
        ...
        <button className="sheet-action" onClick={() => contact('group')} disabled={!person.consent}><House /><span><strong>Convidar para {labels.group}</strong><small>Mensagem de convite sem pressão</small></span><ChevronRight /></button>
        <button className="sheet-action" onClick={() => contact('discipleship')} disabled={!person.consent}><Leaf /><span><strong>Iniciar {labels.discipleship}</strong><small>Apresentar a jornada de encontros</small></span><ChevronRight /></button>
...
function GlobalSearch({ people, groups, labels, close, openPerson, go }: { people: Person[]; groups: SmallGroup[]; labels: AppLabels; close: () => void; openPerson: (person: Person) => void; go: (view: View) => void }) {
  const stageLabels = stageLabelsFor(labels)
  const [query, setQuery] = useState('')
