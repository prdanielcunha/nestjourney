...
describe('My Today factual inbox', () => {
  it('prioritizes only source-backed operational items', () => {
    const items = buildMyTodayItems({ people, careRequests: care, groups, discipleships, sessions, actorId: 'u1', broadAccess: false, now: new Date('2026-09-05T10:00:00Z') })
    expect(items.map((item) => item.kind)).toEqual(['care_debt', 'presence_open', 'group_attention', 'discipleship_next'])
